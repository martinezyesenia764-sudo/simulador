...
// GENERADOR AUTOMÁTICO DE OPCIONES SEGÚN IHL
function updateOptionsFromIHL() {
  const ihlValue = parseInt(ihlInput.value) || 5;
  const extraWords = ihlValue - 5;  // cada palabra = 4 bytes
  const totalHexChars = extraWords * 8; // 4 bytes → 8 hex
  ...
  // Generar automáticamente ABCDEF01 repetido
  let hex = "";
  while (hex.length < totalHexChars) hex += "ABCDEF01";
  hex = hex.slice(0, totalHexChars);

  optionsInput.value = hex;
}
...
ihlInput.addEventListener("input", e => {
  ...
  e.target.value = val;
  updateOptionsFromIHL();
});

updateOptionsFromIHL();

// Validación del input options si el usuario lo edita manualmente
